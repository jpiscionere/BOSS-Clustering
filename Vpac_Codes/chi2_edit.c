...
#include "/home/piscioja/Clustering/WthetaPaper/Source/chisquared_function.h"



double chi2(int lum_sample,double *params) 
 
{
	...
	double logMmin,siglogM,logM0,logM1,alpha,gamma,fgal,tag;
	//makemock parameter
	...
	snprintf(file_tag,MAXLEN,"%5.3f",tag);
	//makemock chars
	...
	snprintf(data_directory,MAXLEN,"/home/piscioja/Clustering/WthetaPaper/Data");	
        ...
        snprintf(covariance_file,MAXLEN,"%s/Wtheta/CovarWtheta_vollim_Mr%d_fib0.20rand.short.inv",data_directory,lum_sample);
	...
	//makemock
	snprintf(galaxy_file,MAXLEN,"%s/halobis_fof_nfw_%s_galaxies",halo_directory,file_tag);	
	
	//stomp
	snprintf(output_tag,MAXLEN,"stomp_sdssmock_main20_esmeralda.%s.f%5.4f.g%5.4f.fof.lss_geometry",file_tag,fgal,gamma);		
