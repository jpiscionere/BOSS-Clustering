...
//function declarations -> you might want to package them into utils.h
...
//general utilities
// MS: 06/14/2012 - check_string_copy is now obsolete.
...
//MS: 06/14/2012 - check_string_copy is now obsolete. Replaced by my_snprintf()

/*
Check that the allocated string size was enough to actually hold the string. 

I envision the call to this function to be something like:

----------------------
nwritten = snprintf(dest,allocated_length,"%s..", var1, var2);
check_string_copy(nwritten,allocated_length);
----------------------

Ohh and you should really use snprintf and avoid those buffer overflows.
*/

/* inline void check_string_copy(const int nwritten,const size_t allocated_length) */
/* {   */
/*   size_t nw=nwritten;//to get rid of the compilation warning.  */
/*   if(nw > allocated_length) */
/* 	{ */
/* 	  fprintf(stderr,"ERROR: String length for snapshot name is %d is too large for the allocated size = %zu \n", */
/* 			  nwritten,allocated_length); */
/* 	  exit(EXIT_FAILURE); */
/* 	} */
/* } */
