...
p<-ggplot(data,aes(x=Z,y=MagR)) + geom_density2d(aes(colour=Bin),alpha=1/150)  + geom_point(alpha=1/150)
...
p<-p + theme_bw() + ggtitle("Redshift vs Absolute Magnitude") + xlab("Redshift") + ylab("Absolute R-Band Magnitude")
...
p<-p + theme(axis.title.y=element_text(size=19,angle=90,vjust=0.7),axis.title.x=element_text(size=20,vjust=-0.2),axis.text=element_text(size=15),panel.border=element_rect(size=1))
